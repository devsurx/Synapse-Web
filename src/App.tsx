...
// --- NEW COMPONENT: MOBILE ANNOUNCEMENT POPUP ---
function MobileAppPopup({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div 
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          className="fixed inset-0 z-[400] flex items-center justify-center bg-black/80 backdrop-blur-md p-6"
        >
          <motion.div 
            initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0 }}
            className="w-full max-w-sm bg-[#0a0a0a] border border-[#96AD8D]/30 rounded-[2.5rem] p-10 text-center shadow-[0_0_50px_rgba(150,173,141,0.1)]"
          >
            <div className="inline-flex p-4 rounded-full bg-[#96AD8D]/10 text-[#96AD8D] mb-6">
              <Smartphone size={40} />
            </div>
            <h3 className="text-xl font-bold mb-3 tracking-tight">Synapse Mobile</h3>
            <p className="text-zinc-400 text-sm leading-relaxed mb-8">
              Take your neural focus engine on the go. Our native mobile app is now available for iOS and Android.
            </p>
            <button 
              onClick={onClose}
              className="w-full py-4 bg-[#96AD8D] text-black font-black uppercase tracking-widest text-[10px] rounded-2xl hover:brightness-110 transition-all flex items-center justify-center gap-2"
            >
              Continue to Web <CheckCircle2 size={14} />
            </button>
          </motion.div>
        </motion.div>
      )}
...
export default function App() {
  ...
  const triggerProactiveAI = async (type: 'finish' | 'motivate') => {
    ...
    try {
      const prompt = type === 'finish' 
        ? "The user just successfully finished a focus timer session. Give them a short, 1-2 sentence positive reinforcement message."
        : "The user just paused their focus timer. Give them a short, gentle 1-sentence motivational nudge to resume.";
  ...
  const toggleTimer = () => {
    if (isActive && seconds < (config[mode] * 60) - 30) triggerProactiveAI('motivate');
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      setAttachedFile({ name: file.name, type: file.type, data: result });
      setChat(p => [...p, { role: 'a', t: `File attached: ${file.name}. Ask me questions about it.`}]);
    };
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setChat(p => [...p, { role: 'a', t: "Sync lost. File might be too large." }]); 
  ...
  return (
    <div className="h-[100dvh] w-full bg-black text-white flex flex-col font-sans overflow-hidden relative">
      ...
      <AnimatePresence mode="wait">
        {loading ? (
          <SplashScreen key="loader" onComplete={() => {
            setLoading(false);
            setShowMobilePopup(true); // Triggers Mobile Popup after Splash
          }} />
        ) : (
          <>
            <MobileAppPopup isOpen={showMobilePopup} onClose={() => setShowMobilePopup(false)} />
            {!user ? (
                  ...
                  <section className="flex-[2] flex flex-col bg-black/50 backdrop-blur-3xl overflow-hidden">
                    ...
                    <div className="p-4 lg:p-8 border-t border-white/5 bg-black/60 shrink-0">
                      ...
                      <form onSubmit={handleSend} className="relative flex items-center">
                        ...
                        <input type="file" ref={fileInputRef} onChange={handleFileUpload} className="hidden" accept=".txt,.md,.csv,.json,image/png,image/jpeg,image/webp" />
                        <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Neural command..." className="w-full bg-white/[0.04] border border-white/10 rounded-2xl py-5 pl-14 pr-16 text-sm text-white focus:outline-none focus:border-[#96AD8D]/40 transition-all shadow-inner" />
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[300] flex items-center justify-center bg-black/90 backdrop-blur-xl p-4">
          <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="w-full max-sm bg-[#080808] border border-white/10 rounded-[2.5rem] lg:rounded-[3rem] p-8 lg:p-12 relative shadow-2xl">
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none bg-[#010102] z-0">
      ...
      {stars.map((star) => (
        ...
        >
          <motion.div className="rounded-full" style={{ width: `${star.size}px`, height: `${star.size}px`, backgroundColor: star.color, boxShadow: star.size > 1.5 ? `0 0 6px ${star.color}` : 'none' }} animate={star.twinkle ? { opacity: [0.3, 1, 0.3] } : {}} transition={{ duration: star.twinkleDur, repeat: Infinity, ease: "easeInOut" }} />
        </motion.div>
      ))}
    </div>
  );
  ...
  return (
    <motion.div initial={false} animate={{ x: isOpen ? 0 : '100%' }} className="fixed right-0 top-24 lg:top-1/2 lg:-translate-y-1/2 z-[60] flex items-center">
      ...
      <div className="w-[300px] lg:w-[340px] h-[300px] lg:h-[380px] bg-[#020203] backdrop-blur-3xl border border-white/10 p-2 rounded-l-3xl shadow-2xl flex flex-col transform translate-x-0">
        <iframe style={{ borderRadius: '20px' }} src="https://open.spotify.com/embed/playlist/37i9dQZF1DX8Ueb9CidzhR" width="100%" height="100%" frameBorder="0" allowFullScreen={false} allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>
      </div>
    </motion.div>
  );
}